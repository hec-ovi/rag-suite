...
import {
  ...
  listRagProjects,
  listRagSessions,
  streamSessionRerankedChat,
  streamStatelessRerankedChat,
  updateRagSession,
} from "../services/rag-reranked.service"
import type {
  RagChatMessage,
  RagChatMode,
  RagChatResponse,
...
export interface RagRerankedState {
...
export interface RagRerankedActions {
...
export function useRagRerankedWorkflow(): { state: RagRerankedState; actions: RagRerankedActions } {
  ...
  const [rerankModelOverride, setRerankModelOverride] = useState("")
  ...
  async function sendMessage(): Promise<void> {
    ...
    setErrorMessage("")
    setStatusMessage("Running hybrid retrieval + rerank and generating answer...")
    setDraftMessage("")
    ...
    try {
      ...
      if (chatMode === "session") {
        ...
        response = await streamSessionRerankedChat(
          payload,
          {
            onDelta: ({ content }) => {
        ...
        response = await streamStatelessRerankedChat(
          payload,
          {
            onDelta: ({ content }) => {
              assembledAnswer += content
  ...
  const actions: RagRerankedActions = {
    selectProject,
    toggleDocument,
