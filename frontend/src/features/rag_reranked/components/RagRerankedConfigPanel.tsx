...
interface RagRerankedConfigPanelProps {
  isOpen: boolean
  onClose: () => void
...
export function RagRerankedConfigPanel({
  isOpen,
  onClose,
  projects,
  selectedProjectId,
  ...
  const [draftRerankCandidateCount, setDraftRerankCandidateCount] = useState(rerankCandidateCount)
  ...
  const [draftRerankModel, setDraftRerankModel] = useState(rerankModelOverride)
  ...
  useEffect(() => {
    ...
    setDraftRerankCandidateCount(rerankCandidateCount)
    ...
    setDraftRerankModel(rerankModelOverride)
  }, [
    isOpen,
    topK,
    denseTopK,
            ...
            <section className="border border-border bg-background p-3">
              ...
              <div className="grid gap-3">
                <div className="grid gap-2 md:grid-cols-2">
                  ...
                  <label className="grid gap-1 text-sm text-muted">
                    <span className="font-medium text-foreground">Rerank Candidates</span>
                    <input
                      type="number"
                      min={1}
                      max={100}
                      value={draftRerankCandidateCount}
                      onChange={(event) => setDraftRerankCandidateCount(parseNumericInput(event.target.value))}
                      disabled={disabled}
                      className="border border-border bg-background px-3 py-2 text-foreground"
                    />
                    <span className="text-xs text-muted">For stricter filtering, keep this higher than Top K.</span>
                  </label>
                ...
                <div className="grid gap-2 md:grid-cols-2">
                  ...
                  <label className="grid gap-1 text-sm text-muted">
                    <span className="font-medium text-foreground">Rerank Model Override</span>
                    <input
                      value={draftRerankModel}
                      onChange={(event) => setDraftRerankModel(event.target.value)}
                      disabled={disabled}
                      placeholder="optional"
                      className="border border-border bg-background px-3 py-2 text-foreground"
                    />
