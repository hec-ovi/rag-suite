...
from src.reranked.models import RagRerankedChatRequest, RagRerankedSessionChatRequest
from src.tools.citation_parser import CitationParser


class _FakeRerankedGraphService:
    ...
    def prepare_stream_stateless(self, state):  # noqa: ANN001, ANN201
        ...
        stream_state["hybrid_candidates"] = []
        ...
        stream_state["rerank_model"] = "bge-reranker-v2-m3:latest"
...
class _FakeRerankedSessionStore:
    def __init__(self) -> None:
...
def test_reranked_stream_chat_stateless_yields_meta_delta_done() -> None:
    service = RagRerankedChatService(
        graph_service=_FakeRerankedGraphService(),  # type: ignore[arg-type]
        citation_parser=CitationParser(),
        default_chat_model="gpt-oss:20b",
        default_embedding_model="bge-m3:latest",
        default_rerank_model="bge-reranker-v2-m3:latest",
...
def test_reranked_stream_chat_session_persists_turn() -> None:
    graph = _FakeRerankedGraphService()
    session_store = _FakeRerankedSessionStore()
    service = RagRerankedChatService(
